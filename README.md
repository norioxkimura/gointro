...
<details><ul><li>配列への参照を取り扱いたい場合はスライスを用います。
</ul></details>
...
s = a[-3:] // エラー
```
- スライス式は `:` の前に開始オフセットを、終端オフセットを指定して、元の配列の一部を参照できます。
- 元の配列の開始オフセットの要素で始まり、長さが「*終端オフセット* - *開始オフセット*」の部分列がスライスになります。
- 省略すると、開始オフセットは 0, 終端オフセットは「`len(a)`」となります。
- 開始オフセットや終端オフセットに負の値を指定して「後ろから～番目」などを指定する機能はありません。
...
s := make([]int, n)
```
- 実行時に決まる長さのスライスを生成したい場合には組み込み関数 `make()` を使います。
- 要素はゼロ値で初期化されます。

## スライス (7) 組み込み関数 `append()`
```go
s := []int{0, 1, 2}
t := append(s, 3, 4, 5)
fmt.Println(s) // → [0 1 2]
fmt.Println(t) // → [0 1 2 3 4 5]
```
- 組み込み関数 `append()` でスライスに要素を追加できます。
- `append()` は渡されたスライスに要素が追加された新しいスライスを返します。
...
t := []int{3, 4, 5}
u := append(s, t...)
fmt.Println(u) // → [0 1 2 3 4 5]
```
- 組み込み関数 `append()` は可変長引数をとる関数（後述）なので、`...` を用いてスライスを引数としてとることができます。
...
```
- スライス式「*スライス*`[:]`」でスライスからスライスを生成できます。
- 生成されたスライスの元の配列は、元のスライスが元とする配列になります。
- つまり２つのスライスで元の配列が共有されます。
...
fmt.Println(t) // → [1 2 3]
```
- 組み込み関数 `copy(`*コピー先スライス*`, `*コピー元スライス*`)` でスライスからスライスへ要素をコピーすることができます。
- `make()` と `copy()` で元の配列を共有しない独立したスライスの複製を生成することができます。

...
<details><ul><li>識別子はレキシカルスコープを持ちます。スコープは最内のブロック（中括弧 <code>{}</code> で区切られたソースコード上の区画）に渡ります。
<ul><li>関数内で宣言された変数名、定数名、型名のスコープは宣言位置以前には渡りません。
<ul><li>それ以外は最内ブロック全体に渡ります。
</details>
